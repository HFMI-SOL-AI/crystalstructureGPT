...
max_iters = 4_000  # total number of training iterations
weight_decay = 5e-2
beta1 = 0.9
beta2 = 0.95
...
angle_sigma_bins = 3.0  # smoothing width (in bins) for lattice angles
fractional_sigma_bins = 4.0  # smoothing width (in bins) for fractional coordinates
...
def get_batch(split):
    ...
    if not valid_lengths:
        raise ValueError(
            "All sequences are too short; ensure tokenised data contains at least two tokens."
        )
...
def estimate_loss():
    ...
    for split in ["train", "val"]:
        ...
        for k in range(eval_iters):
            embeddings, X, Y = get_batch(split)
            with ctx:
                logits, loss = model(X, Y, embeddings=embeddings)
            losses[k] = loss.item()
...
# training loop
embeddings, X, Y = get_batch("train")  # fetch the very first batch
t0 = time.time()
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        embeddings, X, Y = get_batch("train")
