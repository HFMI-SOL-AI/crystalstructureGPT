...
constraint_builder = (
    CrystalConstraintBuilder.from_meta(meta) if use_constraints else None
)
...
def generate_from_embedding(
    ...
    with torch.no_grad():
        with ctx:
            constraint_state = (
                constraint_builder.new_state(start_tokens[0].tolist())
                if constraint_builder
                else None
            )
            constraint_list = (
                [constraint_state] if constraint_state is not None else None
            )
